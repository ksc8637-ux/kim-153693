...
async function getThemeStocks() {
  const res = await fetch('https://api.kiwoom.com/api/dostk/thme', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({ mrkt_tp: '0', inds_tp: '0' })
  });
...
async function getVolumeRanking() {
  const res = await fetch('https://api.kiwoom.com/api/dostk/rkinfo', {
    ...
    body: JSON.stringify({
      ...
      pri_range_strt: '0',
      pri_range_end: '0',
      trde_prica_range_strt: '0',
      trde_prica_range_end: '0'
    })
  });
  return await res.json();
}
...
async function main() {
  try {
    ...
    console.log(`거래량 상위 종목 ${stocks.length}개 조회`);

    const top7 = stocks.slice(0, 7);
    
    let message = '🌏 미국 시장\n\n';
    message += '🎯 오늘 핫 테마 (키움 집계)\n\n';
    message += '⚡ 종가매매 후보 TOP7\n\n';
    ...
    for (let i = 0; i < top7.length; i++) {
      ...
      const entry = price;
      const stop = Math.round(entry * 0.94);
      const stopGap = Math.round(entry * 0.97);
      const target = Math.round(entry * 1.05);

      message += `⭐ ${i+1}위. ${s.stk_nm}\n`;
      message += `현재가: ${formatNumber(price)}원 | 등락: ${rate > 0 ? '+' : ''}${rate}%\n`;
      message += `거래량: ${formatNumber(vol)}\n`;
      message += `✅ 진입가: ${formatNumber(entry)}원 (종가 매수)\n`;
    ...
    message += '🔴 철칙\n';
    message += '✅ 미수 절대 금지\n';
    message += '⚡ 오늘 못 잡으면 내일 또 온다';

    console.log('메시지 생성 완료, 텔레그램 발송 중...');
    ...
    await sendTelegram(CHAT_ID, `❌ 주식비서 오류: ${err.message}`).catch(() => {});
  }
}
